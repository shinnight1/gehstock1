...
(function (SG) {
  var UI=SG.ui, R=SG.gehstockmon, D=R.daten, E=R.wirtschaft, A=R.arena;
  ...
  function mount(host) {
    ...
    var selected=1, battle=null, visual=null, lastNear=null, lastPoll=0, timeOffset=0, tickCount=0, animationToken=0, requestEpoch=0, polling=false;
    ...
    var dock=el('div',undefined,'gm-dock');dock.appendChild(button('▦ Mons',showCollection));dock.appendChild(button('◉ Eier',showEggs));dock.appendChild(button('⚑ Außenposten',showOutposts));dock.appendChild(button('⌖ Weltkarte',function(){if(!connected||busy||battle)return;closeDrawer();root.classList.add('gm-overview');if(world)world.overview();}));root.appendChild(dock);
    ...
    joystick.addEventListener('pointerdown',function(e){if(!connected||joyId!==null||busy||battle||!drawer.hidden)return;joyId=e.pointerId;joystick.setPointerCapture(joyId);joyMove(e);});joystick.addEventListener('pointermove',joyMove);
    ...
    host.onLeave(joyEnd);
    function syncInput(){if(world)world.blockInput(!connected||busy||!!battle||!drawer.hidden);root.setAttribute('aria-busy',String(busy));}
    ...
    function updateResources(){gold.textContent=online?'● '+st.gold+' Gold · +'+income()+'/Std.':'Gemeinsame Spielerwelt';owned.textContent=online?st.besitz.length+'/30 Mons':'Fortschritt wird geladen …';}
    ...
    function update(){
      ...
      if(world&&online){world.setHeld(ts.filter(own).map(function(t){return t.id;}));world.setSquad(st.truppe.map(D.mon));if(world.setTerritories)world.setTerritories(ts,online.playerId);}
      drawTarget();syncInput();
    }
    function selectField(id,center){if(!connected||busy||battle||!drawer.hidden)return;selected=id;lastNear=null;root.classList.remove('gm-overview');if(world)world.select(id,center);update();}
    function drawTarget(){
      UI.clear(target);if(!online||battle)return;var t=current(),f=D.FELDER[selected-1],level=E.LEVELS[t.level];
      ...
      target.appendChild(el('p',f.biom+' · '+(t.ownerId?'Besitzer: ':'Verteidigt von ')+t.ownerName,'gm-territory-owner'));
      ...
      if(own(t)){var production=el('p');production.appendChild(el('span',(t.eggStock||0)+'/3 Eier bereit · '));deadline(production,t.eggAt+E.EGG_TIME,'Ei bereit');target.appendChild(production);}
    ...
    function showCollection(){if(!openDrawer('Deine Mons · '+st.besitz.length+'/30','mons'))return;drawer.appendChild(el('p','Deine Truppe · Das erste Mon beginnt jeden Arenakampf.'));
    ...
    function showPost(id){var t=territories()[id-1];if(!own(t)){showOutposts();notify('Dieser Außenposten gehört inzwischen einem anderen Spieler.');return;}if(!openDrawer(D.FELDER[id-1].name,'post:'+id))return;var l=E.LEVELS[t.level];drawer.appendChild(el('div',undefined,'gm-post-illustration level-'+t.level));drawer.appendChild(el('h3',l.name+' · Stufe '+t.level));drawer.appendChild(el('p','+'+l.income+' Gold/Std. · Verteidiger: +'+Math.round(l.bonus*100)+' % KP, +'+Math.round(l.bonus*50)+' % Angriff.'));var row=el('p',t.eggStock+'/3 Eier bereit. Nächstes Ei: ');deadline(row,t.eggAt+E.EGG_TIME,'bereit');drawer.appendChild(row);
    ...
    function showEggs(){if(!openDrawer('Brutstation','eggs'))return;var occupied=st.eggs.filter(function(e){return e.startedAt!==null;}).length;drawer.appendChild(el('p',st.eggs.length+'/12 Eier · '+occupied+'/3 Brutplätze belegt · Brutzeit 1 Stunde'));if(!st.eggs.length)drawer.appendChild(el('p','Deine Außenposten produzieren alle 2 Stunden ein Ei. Hole fertige Eier dort ab.'));
      ...
      });if(st.besitz.length===30)drawer.appendChild(el('p','Sammlung vollständig: Jedes weitere ausgebrütete Ei bringt 75 Gold.'));
    }
    function refreshDrawer(view){if(view==='eggs')showEggs();else if(view==='posts')showOutposts();else if(view&&view.indexOf('post:')===0)showPost(Number(view.slice(5)));else if(view==='mons')showCollection();}
    function perform(op,data,view){if(!connected||busy||battle)return;view=view||drawerView;
    ...
    function startBattle(){if(!connected||busy||battle)return;var t=current();if(own(t))return;closeDrawer();joyEnd();
    ...
    function renderArena(entry){
      UI.clear(arenaBox);if(!battle)return;var state=visual||battle,f=D.FELDER[battle.territoryId-1];arenaBox.className='gm-arena biome-'+((battle.territoryId-1)%5);
      var header=el('header',undefined,'gm-arena-header');header.appendChild(el('div','GEBIETSARENA · '+f.name,'gm-eyebrow'));header.appendChild(el('strong','Runde '+Math.min(battle.round,60)+' · '+E.LEVELS[battle.level].name));arenaBox.appendChild(header);
      ...
      if(battle.phase==='finished'&&!animating){line.textContent=battle.message||(battle.winner==='wir'?'Gebiet erobert!':'Der Kampf ist beendet.');panel.appendChild(el('h2',battle.winner==='wir'?'Dein Revier wächst.':battle.winner==='fled'?'Zurück im Lager.':'Die Verteidigung hält.'));panel.appendChild(button('Zurück zur Karte',closeArena,'gm-button gm-primary'));arenaBox.appendChild(panel);return;}
    ...
    function applyOnline(res){if(dead)return;if(!res.territories||res.territories.length!==D.FELDER.length)throw new Error('Die Karte wurde aktualisiert. Bitte lade die Website neu.');online=res;connected=true;lastPoll=Date.now();timeOffset=res.serverTime-Date.now();st=D.neuerStand(res.profile,res.serverTime);worldButton.textContent=lastPresenceReply?'Spielerwelt · '+(peerList.length+1)+' online':'Spielerwelt';connectionBox.hidden=true;root.classList.remove('gm-disconnected');if(world)world.pause(!!battle);update();}
    function applyPeers(peers,serverTime){if(dead)return;peerList=peers;var keep={};peers.forEach(function(p){keep[p.id]=true;var label=peerLabels[p.id];if(!label){label=el('span',undefined,'gm-peer-label');peerLayer.appendChild(label);peerLabels[p.id]=label;}label.textContent=p.name+(p.activity==='arena'?' · ⚔':'');label.hidden=true;label.title=p.activity==='arena'?'Kämpft gerade in einer Arena':'Auf der Insel unterwegs';});Object.keys(peerLabels).forEach(function(id){if(!keep[id]){peerLabels[id].remove();delete peerLabels[id];}});if(world&&world.setPeers)world.setPeers(peers,serverTime);worldButton.textContent='Spielerwelt · '+(peers.length+1)+' online';}
    function syncPresence(){if(dead||!connected||presenceBusy||document.hidden||!world||!world.position)return;presenceBusy=true;lastPresence=Date.now();R.online.request('presence',{position:world.position()}).then(function(res){if(dead||!connected)return;lastPresenceReply=Date.now();applyPeers(res.peers||[],res.serverTime);}).catch(function(){if(!dead&&Date.now()-lastPresenceReply>15000){applyPeers([],now());worldButton.textContent='Spielerwelt · Verbindung prüfen';}}).finally(function(){presenceBusy=false;});}
    ...
    function onlineError(err){if(dead)return;if(!connected||!online||!err.status||err.status===401||err.status>=500||err.status===408||err.status===429){showConnection(err.message||'Der Server ist nicht erreichbar.');return;}notify(err.message);if(battle)renderArena(false);}
    ...
    function resumeOnline(){if(busy)return;requestOnline(R.online.pending()?'resume':'world').then(function(res){applyOnline(res);if(res.arena)showArena(res.arena,false);else{showOnline();notify(res.message||'Aktueller Stand geladen.');}}).catch(onlineError);}
    ...
    try{world=R.createWorld(host,worldBox,{
      ...
      frame:function(project,inBattle,position,visiblePeers){pins.forEach(function(pin,i){var o=R.orte[i],p=project({x:o.x,y:6,z:o.z});pin.style.transform='translate('+p.x+'px,'+p.y+'px) translate(-50%,-100%)';pin.hidden=!online||!!battle||!p.visible||(!root.classList.contains('gm-overview')&&!p.near&&i+1!==selected);});Object.keys(peerLabels).forEach(function(id){peerLabels[id].hidden=true;});(visiblePeers||[]).forEach(function(peer){var label=peerLabels[peer.id];if(!label)return;var p=project({x:peer.position.x,y:4.2,z:peer.position.z});label.hidden=!!battle||!connected||!p.visible||(!p.near&&!root.classList.contains('gm-overview'));label.style.transform='translate('+p.x+'px,'+p.y+'px) translate(-50%,-100%)';});if(!battle&&world){var near=world.distanceTo(selected)<10;if(near!==lastNear){lastNear=near;drawTarget();}}},
    ...
    function tick(){if(dead)return;tickCount++;updateResources();
      ...
      if(connected&&tickCount%15===0&&!battle){drawTarget();if(drawerView==='posts'||drawerView&&drawerView.indexOf('post:')===0)refreshDrawer(drawerView);}
      if(connected&&!busy&&!battle&&!polling&&document.hidden===false&&Date.now()-lastPoll>30000){lastPoll=Date.now();polling=true;var epoch=requestEpoch;R.online.request('world').then(function(res){if(dead||!connected||epoch!==requestEpoch||busy||battle)return;applyOnline(res);if(res.arena&&res.arena.phase!=='finished')showArena(res.arena,false);else if(drawerView==='posts'||drawerView&&drawerView.indexOf('post:')===0)refreshDrawer(drawerView);}).catch(function(err){if(epoch===requestEpoch)onlineError(err);}).finally(function(){polling=false;});}
      host.after(tick,1000);
    }
    update();connectWorld();host.after(tick,1000);
    return {get state(){return st;},selftest:function(){if(D.KATALOG.length!==30)throw new Error('30 Mons erwartet');},destroy:function(){dead=true;requestEpoch++;animationToken++;window.removeEventListener('blur',joyEnd);window.removeEventListener('offline',connectionLost);window.removeEventListener('online',connectionRestored);document.removeEventListener('visibilitychange',joyEnd);if(world)world.destroy();}};
  }
  SG.register({id:'gehstockmon',name:'GehstockMon',category:'tycoon',onlineOnly:true,heavy:true,desc:'Gemeinsame Online-Welt: 30 Mons, Arenakämpfe und Außenposten',tags:['3d','monster','arena','eier','revier','spielerwelt'],
