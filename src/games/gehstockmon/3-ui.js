...
(function (SG) {
  var UI=SG.ui, R=SG.gehstockmon, D=R.daten, E=R.wirtschaft, A=R.arena, H=R.zeiten;
  ...
  function mount(host) {
    ...
    function applyOnline(res){if(dead)return;if(!res.territories||res.territories.length!==D.FELDER.length)throw new Error('Die Karte wurde aktualisiert. Bitte lade die Website neu.');setAccess(res.access);online=res;connected=true;lastPoll=Date.now();timeOffset=res.serverTime-Date.now();st=D.neuerStand(res.profile,res.serverTime);worldButton.textContent=lastPresenceReply?'Spielerwelt · '+(peerList.length+1)+' online':'Spielerwelt';connectionBox.hidden=true;root.classList.remove('gm-disconnected');if(world)world.pause(!!battle);update();if(res.weekendDelivery&&!res.duplicate)notify(res.weekendDelivery+' Wochenend-Eier sind in deiner Bruttasche angekommen.');}
    ...
    function syncPresence(){if(dead||!connected||presenceBusy||document.hidden||!world||!world.position)return;presenceBusy=true;lastPresence=Date.now();R.online.request('presence',{position:world.position()}).then(function(res){if(dead||!connected)return;setAccess(res.access);lastPresenceReply=Date.now();applyPeers(res.peers||[],res.serverTime);}).catch(function(err){if(err.status===423){onlineError(err);return;}if(!dead&&Date.now()-lastPresenceReply>15000){applyPeers([],now());worldButton.textContent='Spielerwelt · Verbindung prüfen';}}).finally(function(){presenceBusy=false;});}
    ...
    return {get state(){return st;},selftest:function(){if(D.KATALOG.length!==30)throw new Error('30 Mons erwartet');},destroy:function(){dead=true;requestEpoch++;animationToken++;if(closeTimer)host.cancel(closeTimer);window.removeEventListener('blur',joyEnd);window.removeEventListener('offline',connectionLost);window.removeEventListener('online',connectionRestored);document.removeEventListener('visibilitychange',joyEnd);if(world)world.destroy();}};
  }
