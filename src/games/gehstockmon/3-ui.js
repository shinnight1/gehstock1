...
(function (SG) {
  ...
  function art(mon) { return UI.el('img.gm-portrait'+(mon&&mon.schimmernd?'.gm-schimmer':''),{src:SG.assets[mon.bild],alt:mon.name,draggable:false}); }
  /* Dasselbe Portrait, aber mit dem Schaubild, wo es eins gibt. Nur in der
     Sammlung - im Kampf und auf der Karte wuerde der Hintergrund stoeren. */
  function artGross(mon) { return UI.el('img.gm-portrait'+(mon&&mon.schimmernd?'.gm-schimmer':''),{src:SG.assets[mon.vorschau]||SG.assets[mon.bild],alt:mon.name,draggable:false}); }
  ...
  function mount(host) {
    ...
    var dock=el('div',undefined,'gm-dock');dock.appendChild(button('▦ Mons',showCollection));dock.appendChild(button('◉ Eier',showEggs));dock.appendChild(button('◆ Dungeons',function(){adventures.dungeons();}));dock.appendChild(button('✦ Abenteuer',function(){adventures.adventure();}));dock.appendChild(button('⚒ Ausrüstung',function(){adventures.shop();}));dock.appendChild(button('♛ Stockhafen',function(){stadt.menu();}));dock.appendChild(button('⚑ Außenposten',showOutposts));dock.appendChild(button('⌖ Weltkarte',function(){if(!connected||busy||battle||dueling())return;closeDrawer();root.classList.add('gm-overview');if(world)world.overview();}));root.appendChild(dock);
    ...
    function drawTarget(){
      ...
      var badges=el('div',undefined,'gm-target-badges');badges.appendChild(el('span',level.name+' · Stufe '+t.level));
      /* Wer weniger Land haelt als sein Ziel, schlaegt haerter zu - das soll
         man vorher sehen und nicht erst im Kampf merken. */
      if(!own(t)&&t.ownerId){
    ...
    function showHelp(){if(!openDrawer('So wächst dein Revier'))return;
      drawer.appendChild(el('h3','Öffnungszeiten · deutsche Ortszeit'));H.LABELS.forEach(function(line){drawer.appendChild(el('p',line));});drawer.appendChild(el('p','Am Wochenende ruht die normale Eierproduktion. Jeder gehaltene Außenposten bringt zwei Wochenend-Eier. Beim nächsten Eintritt werden sie gutgeschrieben; bei voller Tasche bleiben sie reserviert. Gold pro Stunde pausiert in jeder Ruhezeit. Um 0:00 Uhr werden täglich 150 Gold je gehaltenem Gebiet reserviert und beim nächsten Eintritt ausgezahlt. Laufende Brutzeiten laufen weiter.'));
      [['Neue Abenteuer','Zwei Wandertrainer ziehen stündlich durch die weite Graslandschaft und schenken dir für Siege Eier. Sammle Runen und erkunde Biome für Quests. In Ausrüstung kaufst du Skins und Waffen.'],['Überfälle','Tippe auf einen Mitspieler. Gewinne erst das Waffenduell gegen seine gespeicherte Ausrüstung und danach den Mon-Kampf, um ein Ei zu erbeuten. Einen Anfängerschutz gibt es nicht: ein Ei trägst du von der ersten Minute an auf eigenes Risiko. Nach einem Diebstahl gelten 2 Stunden Schutz, und zwischen zwei eigenen Überfällen liegt eine halbe Stunde.'],['Erkunden','Laufe mit dem Joystick, WASD oder einem Tipp auf den Boden. Ziehen verschiebt die Kamera. Zwei Finger zoomen. ◎ bringt dich zur Figur.'],['Arenakämpfe','In der Arena kämpft ein Mon pro Seite. Wähle Stockhieb, Kraftschlag, Spezialattacke oder Deckung. Schnellere Mons greifen zuerst an. Ein freiwilliger Wechsel verbraucht deinen Zug; nach einem K. o. ist der Wechsel frei.'],['Eier und Brut','Jeder eigene Außenposten liefert nach 2 Stunden Öffnungszeit ein Ei und lagert höchstens 3. Während die Insel ruht, pausiert die Produktion; angefangene Produktionszeit bleibt erhalten. Hole sie ab und starte die einstündige Brut. Du beginnst mit 3 Brutplätzen und hast Platz für 12 Eier. Der fertige Leuchtturm bringt einen vierten Platz, drei weitere kaufst du in Stockhafen mit Gold. Gesammelte Eier bleiben bei dir, auch wenn du das Gebiet verlierst.'],['Gold und Ausbau','Ein Lager verdient 20 Gold pro Stunde, ein Wachtposten 35, eine Festung 55. Gold entsteht während deiner Abwesenheit ausschließlich in den Öffnungszeiten. Zusätzlich gibt es täglich um 0:00 Uhr 150 Gold je gehaltenem Gebiet zur nächsten Anmeldung. Die Ausbauten kosten 120 und 300 Gold. Sie erhöhen die KP der Verteidiger um 12 % und 25 % sowie deren Angriff um 6 % und 12,5 %.'],['Kampfteam und Besatzungen','Jeder Außenposten kann seine eigene Besatzung aus vier Mons haben. Dasselbe Mon darf dabei an mehreren Stellen zugleich stehen - im Kampfteam und auf beliebig vielen Posten. Solange du keine eigene Besatzung gesetzt hast, springt dein Kampfteam ein. Besatzungen setzt du unter ⚑ Außenposten beim jeweiligen Gebiet; ein Knopf stellt die stärkste Truppe auf alle offenen Posten.'],['Wesen','Jedes Mon bringt beim Schlüpfen ein Wesen mit, und das ändert seine Werte um einige Prozent: ein wildes schlägt härter zu und hält weniger aus, ein stures umgekehrt, ein flinkes kommt früher zum Zug. Zwei Donnerwidder sind damit nicht mehr dasselbe — und am Tauschbrett lohnt es sich, darauf zu achten.'],['Kämpfe planen','Jedes Mon hat drei Wenn-Dann-Regeln. Sie greifen, wenn du nicht selbst am Zug bist: wenn deine Außenposten angegriffen werden, während du offline bist, und wenn dich jemand in der Arena herausfordert. Von oben nach unten wird die erste Regel genommen, die passt. Den Plan setzt du beim Mon selbst — und beim Aufklären siehst du den Plan des Gegners genauso offen. Das ist der Kern: du liest seine Denkweise, nicht seine Truppenstärke.'],['Rollen und ihre Konter','Jede Rolle steht gegen eine gut und gegen eine schlecht. Wälle dämpfen Schneiden um ein Viertel, Schneiden zerlegen Pfleger, Störer kommen an Wällen vorbei, Pfleger halten gegen Störer. Die Aufstellung ist damit eine Entscheidung und keine Zahlensumme.'],['Zwölf Fähigkeiten','Jede Rolle hat drei verschiedene Fähigkeiten, und jedes Mon beherrscht genau eine davon. Ein Wall wirft entweder einen Schild hoch, zieht einen Steinwall mit Heilung oder lässt Treffer über Dornen zurückfallen. Welches Mon du aufstellst, entscheidet also mit, welche Werkzeuge du im Kampf hast.'],['Außenseiterhilfe','Wer weniger Gebiete hält als sein Ziel, kämpft mit +8 % KP und Angriff je Gebiet Unterschied, höchstens +40 %. Dem Führenden wird nichts weggenommen — er wird nur angreifbar. Im Zielfenster steht vorher, wie viel du bekommst.'],['Tauschbrett','In Stockhafen hängst du ein Mon aus und sagst, was du dafür suchst. Getauscht wird nur innerhalb derselben Seltenheit. Was du weggibst, ist weg — samt Runenstufe und Wesen. Was in deiner Truppe steht, kannst du nicht anbieten.'],['Kampfberichte','Unter Spielerwelt stehen die letzten Gebietskämpfe. Einer mit Verlauf lässt sich antippen und Runde für Runde nachlesen — vor allem, wenn du nachts angegriffen wurdest und wissen willst, woran deine Verteidigung gescheitert ist.'],['Stockhafen und die Große Arena','Die freie Stadt im Westen gehört niemandem und kann nicht erobert werden. In ihrer Mitte steht die Große Arena — massives Gemäuer, durch das man nicht hindurchläuft. Dort kämpfst du gegen die gespeicherte Truppe anderer Spieler, ohne dass sie anwesend sein müssen, und niemand verliert dabei etwas. Nach drei Ranglistensiegen darfst du den Gehstock-Champion herausfordern. Wer ihn schlägt, trägt den Titel, verteidigt ihn mit der eingefrorenen Aufstellung und bekommt täglich Sold. Hält ihn kein Spieler, hält ihn Meister Gehstock.'],['Ohne Gebiet','Wer keinen Außenposten hält, ist nicht raus. Im Hafen von Stockhafen gibt es alle zwei geöffneten Stunden ein Tagwerk für Gold und alle vier Stunden ein Ei aus dem Findelhaus. Dazu kommt das Gold aus der Arena. Sobald du wieder ein Gebiet hältst, schließen beide — dann kommen deine Eier von dort.'],['Eier-Quoten','Aus jedem Ei schlüpft für alle gleich: 40 % Gewöhnlich, 25 % Selten, 17 % Außergewöhnlich, 11 % Episch, 5 % Legendär, 1,6 % Mythisch, 0,4 % Apokalyptisch. Innerhalb einer Seltenheit ist jedes Mon gleich wahrscheinlich, egal was du schon hast. Der Garantie-Zähler in der Brutstation sorgt dafür, dass spätestens jedes zehnte Ei Episch oder besser ist und jedes vierzigste Legendär oder besser. Eins von 64 Mons schlüpft schimmernd - eine seltene Farbvariante.'],['Zwillinge und Runenstufen','Ein Ei kann ein Mon bringen, das du schon hast. Dann geht nichts verloren: beide verschmelzen und die Runenstufe steigt um eins. Steht es schon auf 5/5, zerfällt der Zwilling zu fünf Runen seiner Seltenheit. Jede Stufe gibt +3 % KP und Angriff, ab Stufe 3 lädt der Kraftschlag eine Runde schneller, ab Stufe 4 gibt es eine dritte Ladung der Fähigkeit. Brutplätze kannst du in Stockhafen mit Gold dazukaufen.'],['Spielerwelt','Hier kämpfen echte Hideout-Spieler und Computergegner um dieselben Gebiete. Deine gespeicherte Truppe verteidigt automatisch, wenn du offline bist. Kämpfe und Brutzeiten bleiben auf dem Server gespeichert.']].forEach(function(v){drawer.appendChild(el('h3',v[0]));drawer.appendChild(el('p',v[1]));});
    }
    function showCollection(){if(!openDrawer('Deine Mons · '+st.besitz.length+'/'+D.KATALOG.length+'','mons'))return;drawer.appendChild(el('p','Deine Truppe · Das erste Mon beginnt jeden Arenakampf.'));
      ...
      var grid=el('div',undefined,'gm-collection'),gezeigt=0;D.KATALOG.slice().sort(function(a,b){return b.seltenheit-a.seltenheit||a.name.localeCompare(b.name,'de');}).filter(function(k){
        ...
        return true;
      }).forEach(function(k){gezeigt++;var have=st.besitz.indexOf(k.id)>=0,b=button('',function(){showMon(k.id);},'gm-collect-card'+(have?'':' gm-unowned')+(have&&st.schimmernd&&st.schimmernd[k.id]?' gm-schimmer-rahmen':''));b.style.setProperty('--rarity',D.SELTENHEITEN[k.seltenheit].farbe);b.appendChild(artGross(have?X.mon(st,k.id):k));b.appendChild(el('strong',k.name));b.appendChild(el('span',D.SELTENHEITEN[k.seltenheit].name));
        /* Ein Mon kann an mehreren Stellen zugleich stehen - die Karte nennt das Kampfteam und zaehlt die Posten. */
        var orte=X.einsatzOrte(st,k.id),posten=orte.filter(function(o){return o!=='kampfteam';}).length;
        var dienst=orte.indexOf('kampfteam')>=0?'IM KAMPFTEAM'+(posten?' + '+posten+' POSTEN':''):posten?'AUF '+posten+' POSTEN':'';
        b.appendChild(el('small',dienst||(have?k.rolle:'SCHLÜPFT AUS EINEM EI')));grid.appendChild(b);});
    ...
    function planTeil(id,k){
      ...
      plan.forEach(function(zeile,i){
        var reihe=el('div',undefined,'gm-plan-row');
        reihe.appendChild(el('span',(i+1)+'.','gm-plan-nr'));
        var wenn=el('select'),dann=el('select');
        wenn.setAttribute('aria-label','Regel '+(i+1)+': Bedingung');
        dann.setAttribute('aria-label','Regel '+(i+1)+': Aktion');
        A.PLAN_WENN.forEach(function(w){var o=el('option',w.text);o.value=w.id;wenn.appendChild(o);});
        A.PLAN_DANN.forEach(function(d){var o=el('option',d.text);o.value=d.id;dann.appendChild(o);});
        wenn.value=zeile[0];dann.value=zeile[1];
        wenn.disabled=dann.disabled=busy;
        reihe.appendChild(wenn);reihe.appendChild(el('span','→','gm-plan-pfeil'));reihe.appendChild(dann);
        drawer.appendChild(reihe);felder.push([wenn,dann]);
      });
      var speichern=button('Plan speichern',function(){
        perform('plan',{monId:id,plan:felder.map(function(f){return [f[0].value,f[1].value];})},'mons');
      },'gm-button gm-primary');
      speichern.disabled=busy;
      drawer.appendChild(speichern);
      if(!(st.plaene&&st.plaene[id]))drawer.appendChild(el('p','Noch kein eigener Plan gespeichert - bis dahin kämpft '+k.name+' nach der alten Faustregel.','gm-plan-hinweis'));
    }
    function showMon(id){var k=X.mon(st,id);if(!k||!openDrawer(k.name))return;drawer.style.setProperty('--rarity',D.SELTENHEITEN[k.seltenheit].farbe);var hero=el('div',undefined,'gm-mon-hero');hero.appendChild(artGross(k));hero.appendChild(el('span',D.SELTENHEITEN[k.seltenheit].name,'gm-rarity-label'));drawer.appendChild(hero);drawer.appendChild(el('p',k.lore));var s=A.stats(k);drawer.appendChild(el('p',s.hp+' KP · '+s.ang+' Angriff · '+s.tempo+' Tempo','gm-mon-stats'));
      ...
      var wesen=X.wesenVon(st,id);
      if(wesen)drawer.appendChild(el('p','Wesen: '+wesen.name+' · '+X.wesenText(wesen),'gm-mon-wesen'));
      if(k.schimmernd)drawer.appendChild(el('p','✨ Schimmernd - eine seltene Farbvariante. Eins von 64 Mons schlüpft so.','gm-mon-schimmer'));
      var orte=X.einsatzOrte(st,id);
      if(orte.length)drawer.appendChild(el('p','Im Dienst: '+X.einsatzListe(orte)+'.','gm-mon-dienst'));
      A.moves(u,1).forEach(function(m){drawer.appendChild(el('p',m.name+' · '+m.text));});
      if(st.besitz.indexOf(id)<0)drawer.appendChild(el('p','Brüte Eier aus - aus Außenposten, von Wandertrainern, aus dem Findelhaus oder der Tagestruhe. Die Seltenheit entscheidet das Glück, innerhalb der Seltenheit ist jedes Mon gleich wahrscheinlich.'));
    ...
    function showScout(){var t=current();if(!openDrawer('Arena · '+D.FELDER[selected-1].name))return;drawer.appendChild(el('p',t.ownerName+' · '+E.LEVELS[t.level].name+' · Verteidiger-KP +'+Math.round(E.LEVELS[t.level].bonus*100)+' %'));var team=A.defenders(t.id,t.ownerId?t.defense:null),grid=el('div',undefined,'gm-squad-preview');team.forEach(function(k){var card=button('',function(){showMon(k.id);},'gm-party-card');card.appendChild(art(k));card.appendChild(el('strong',k.name));grid.appendChild(card);});drawer.appendChild(grid);
      /* Der gegnerische Plan liegt offen - das ist der Kern des Spiels und
         ...
         Truppenstaerke. */
      drawer.appendChild(el('h3','Wie sie kämpfen'));
      team.forEach(function(k){
        var u=A.create([k],[k],{}).teams[0][0],karte=el('article',undefined,'gm-plan-karte');
        karte.appendChild(el('h3',k.name+' · '+k.rolle));
        karte.appendChild(el('p',A.faehigkeit(u).name+' · '+rollenText(k.typ)));
        if(A.planGueltig(k.plan))k.plan.forEach(function(z){
          if(z[0]==='aus')return;
          var w=A.PLAN_WENN.find(function(v){return v.id===z[0];}),d=A.PLAN_DANN.find(function(v){return v.id===z[1];});
          if(w&&d)karte.appendChild(el('p',w.text+' → '+d.text,'gm-plan-zeile'));
        });
        else karte.appendChild(el('p','Kein eigener Plan - kämpft nach der Faustregel.','gm-plan-zeile'));
    ...
    function showOutposts(){if(!openDrawer('Deine Außenposten','posts'))return;var ts=territories().filter(own);drawer.appendChild(el('p',ts.length+' Gebiete · '+income()+' Gold pro Stunde. Einnahmen werden automatisch gutgeschrieben.'));if(!ts.length)drawer.appendChild(el('p','Erobere dein erstes Gebiet in einem Arenakampf. Dort beginnen die Gold- und Eierproduktion.'));
      /* Wer neun Posten von Hand besetzt, klickt sechsunddreissig Mal. Ein Knopf
         stellt die staerkste Truppe auf jeden Posten ohne eigene Besatzung und
         laesst gesetzte Besatzungen stehen. */
      if(ts.length){
        var offen=ts.filter(function(t){return X.notbesatzung(st,t.id);});
        drawer.appendChild(el('p',offen.length+' Posten ohne eigene Besatzung · ein Mon darf auf mehreren Posten zugleich stehen'));
        if(offen.length){
          var auto=button('Stärkste Truppe auf alle offenen Posten',function(){perform('besatzung_auto',{},'posts');},'gm-button gm-primary');
          auto.disabled=busy||st.besitz.length<X.TRUPPE;
    ...
    /* Die Besatzung eines Aussenpostens. Vier Mons, die hier Dienst tun - sie
       duerfen zugleich woanders stehen. Wer keine setzt, laesst das Kampfteam
       einspringen. */
    function besatzungTeil(id){
      drawer.appendChild(el('h3','Besatzung'));
      var eigen=X.posten(st,id);
      drawer.appendChild(el('p',eigen
        ? 'Vier eigene Mons halten diesen Posten. Sie dürfen zugleich im Kampfteam oder auf anderen Posten stehen.'
        : 'Kein eigenes Kommando: dein Kampfteam springt ein. Mit einer eigenen Besatzung stellst du hier eine passende Truppe auf.',
        eigen?undefined:'gm-plan-hinweis'));
      var reihe=el('div',undefined,'gm-squad-preview');
      X.besatzung(st,id).forEach(function(mid){
        var k=X.mon(st,mid);if(!k)return;
        var karte=button('',function(){showMon(mid);},'gm-party-card');
        karte.style.setProperty('--rarity',D.SELTENHEITEN[k.seltenheit].farbe);
        karte.appendChild(art(k));karte.appendChild(el('strong',k.name));
        reihe.appendChild(karte);
      });
      drawer.appendChild(reihe);
      /* Zur Wahl steht die ganze Sammlung, die staerksten zuerst. */
      var frei=st.besitz.filter(function(mid){return !!D.mon(mid);}).sort(function(a,b){return X.kampfwert(st,b)-X.kampfwert(st,a);});
      if(eigen){
        var ab=button('Besatzung abziehen',function(){perform('besatzung',{territoryId:id,squad:null},'post:'+id);},'gm-button gm-secondary');
        ab.disabled=busy;drawer.appendChild(ab);
      }
      if(frei.length<X.TRUPPE){
        drawer.appendChild(el('p','Für eine eigene Besatzung brauchst du vier Mons.','gm-plan-hinweis'));
        return;
      }
      var wahl=[];
      for(var platz=0;platz<X.TRUPPE;platz++){
        var zeile=el('div',undefined,'gm-plan-row');
        zeile.appendChild(el('span',(platz+1)+'.','gm-plan-nr'));
        var feld=el('select');
        feld.setAttribute('aria-label','Besatzung Platz '+(platz+1));
        frei.forEach(function(mid){var k=D.mon(mid),o=el('option',k.name+' · '+D.SELTENHEITEN[k.seltenheit].name);o.value=mid;feld.appendChild(o);});
        feld.value=(eigen&&frei.indexOf(eigen[platz])>=0?eigen[platz]:frei[platz])||frei[0];
        feld.disabled=busy;
        zeile.appendChild(feld);drawer.appendChild(zeile);wahl.push(feld);
      }
      var setzen=button(eigen?'Besatzung ändern':'Besatzung aufstellen',function(){
        var squad=wahl.map(function(f){return f.value;});
        if(new Set(squad).size!==X.TRUPPE){notify('Wähle vier verschiedene Mons.');return;}
        perform('besatzung',{territoryId:id,squad:squad},'post:'+id);
      },'gm-button gm-primary');
      setzen.disabled=busy;drawer.appendChild(setzen);
    }
    function showEggs(){if(!openDrawer('Brutstation','eggs'))return;var occupied=st.eggs.filter(function(e){return e.startedAt!==null;}).length,plaetze=(online&&online.stadt&&online.stadt.brutplaetze)||E.INCUBATORS;drawer.appendChild(el('p',st.eggs.length+'/12 Eier · '+occupied+'/'+plaetze+' Brutplätze belegt · Brutzeit 1 Stunde'));if(!st.eggs.length)drawer.appendChild(el('p','Deine Außenposten produzieren nach 2 Stunden Öffnungszeit ein Ei. Während der Ruhezeiten pausiert die Produktion. Hole fertige Eier dort ab.'));
    ...
    function chancenTafel(){
      ...
      chancen.forEach(function(v){
        ...
        zeile.appendChild(el('small',v.offen?v.offen+' offen':'alle da'));
      ...
      /* Der Garantie-Zaehler: wie viele Eier es hoechstens noch dauert. */
      var garantie=el('div',undefined,'gm-garantie');
      E.garantieStand(st).forEach(function(g){
        var r=D.SELTENHEITEN[g.ab],zeile=el('div',undefined,'gm-garantie-zeile');
        zeile.appendChild(el('span',g.noch===1?'Nächstes Ei: garantiert '+r.name+' oder besser':'Spätestens in '+g.noch+' Eiern: garantiert '+r.name+' oder besser'));
        var balken=el('span',undefined,'gm-garantie-balken'),fuellung=el('i');fuellung.style.width=Math.round(g.seit/g.nach*100)+'%';fuellung.style.background=r.farbe;
        balken.appendChild(fuellung);zeile.appendChild(balken);garantie.appendChild(zeile);
      });
      drawer.appendChild(garantie);
      drawer.appendChild(el('p','Die Quoten gelten für alle gleich, egal was du schon hast. Innerhalb einer Seltenheit ist jedes Mon gleich wahrscheinlich; eins von 64 schlüpft schimmernd. Ein Mon, das du schon hast, verschmilzt mit seinem Zwilling und hebt die Runenstufe. Steht es schon auf '+X.UPGRADE_LIMIT+'/'+X.UPGRADE_LIMIT+', zerfällt der Zwilling zu Runen seiner Seltenheit.','gm-chancen-fuss'));
    }
    function refreshDrawer(view){if(view==='bericht')showOnline();else if(view==='stadt')stadt.refresh(view);else if(view==='eggs')showEggs();else if(view==='posts')showOutposts();else if(view&&view.indexOf('post:')===0)showPost(Number(view.slice(5)));else if(view==='mons')showCollection();else if(view==='heute')heute.zeigen();else adventures.refresh(view);}
    ...
    function renderArena(entry){
      ...
      [0,1].forEach(function(side){var u=state.teams[side][state.active[side]],mon=D.mon(u.monId),unit=el('div',undefined,'gm-arena-unit '+(side===0?'ally':'enemy')+(entry?' gm-send':''));unit.setAttribute('data-unit',u.uid);
        ...
        var pedestal=el('div',undefined,'gm-arena-pedestal');unit.appendChild(pedestal);unit.appendChild(el('div',undefined,'gm-throw-orb'));var img=art(Object.assign({},mon,{schimmernd:u.schimmernd}));img.classList.add('gm-arena-mon');if(u.hp<=0)img.classList.add('fainted');unit.appendChild(img);
    ...
    /* Wo die Figur nach dem Verbinden steht. Beim ersten Mal genau dort, wo
       ...
       bleibt sie, wo sie ist: bewegen konnte sie sich ohne Verbindung nicht,
       und die paar Schritte seit der letzten Meldung laesst der Server als
       Weg gelten. Nur wer weit weg steht (Testzone verlassen, anderes
    ...
    function showAdminMenu(card,message){
      ...
      var menu=el('div',undefined,'gm-admin-menu');menu.appendChild(el('strong','Developer-Testzone'));menu.appendChild(el('p','Admin-Code eingeben, um die Insel auch während der Ruhezeit zu öffnen. Nur für echte Hideout-Admins. Andere Admins in der Testzone sind sichtbar und können gemeinsam testen. Die echte Spielerwelt bleibt unverändert.'));
      ...
      var unlock=button('Testzone öffnen',function(){if(input.value!=='3142'){feedback.textContent='Der Code ist nicht korrekt.';input.value='';input.focus();return;}R.adminOverride=true;saveAdmin(true);adminNotice=null;menu.remove();notify('Developer-Testzone aktiviert.');connectWorld();},'gm-button gm-primary');
    ...
    function showOnline(){if(!openDrawer('Die Spielerwelt','online'))return;drawer.appendChild(el('p','Du teilst diese Welt mit allen Hideout-Spielern. Erobere Gebiete von Spielern und Computergegnern. Deine gespeicherten Mons verteidigen auch, wenn du offline bist.'));
      if(R.adminOverride){drawer.appendChild(el('p','GEMEINSAME TESTZONE · Andere Tester sind sichtbar und können mit dir spielen. Alles bleibt flüchtig: Kein Gold, kein Kauf und kein Kampf verändert die echte Spielerwelt.'));drawer.appendChild(button('Testzone verlassen',function(){R.adminOverride=false;saveAdmin(false);R.online.resetTest();online=null;battle=null;connectWorld();}));}drawer.appendChild(el('h3','Auf der Insel'));drawer.appendChild(el('p',peerList.length?peerList.map(function(p){return p.name+(p.activity==='arena'?' (in der Arena)':'');}).join(' · '):'Gerade sind keine anderen Spieler sichtbar.'));
      ...
      drawer.appendChild(el('h3','Letzte Gebietskämpfe'));
      (online.reports||[]).slice().reverse().forEach(function(r){
        ...
        karte.appendChild(el('small',(r.angreifer||[]).join(', ')+' gegen '+(r.verteidiger||[]).join(', ')));
        drawer.appendChild(karte);
      });
      if(!(online.reports||[]).length)drawer.appendChild(el('p','Noch keine Gebietskämpfe.'));
    }
