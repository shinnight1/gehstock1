...
(function (SG) {
  ...
  function mount(host) {
    ...
    function showMon(id){var k=X.mon(st,id);if(!k||!openDrawer(k.name))return;drawer.style.setProperty('--rarity',D.SELTENHEITEN[k.seltenheit].farbe);var hero=el('div',undefined,'gm-mon-hero');hero.appendChild(artGross(k));hero.appendChild(el('span',D.SELTENHEITEN[k.seltenheit].name,'gm-rarity-label'));drawer.appendChild(hero);drawer.appendChild(el('p',k.lore));var s=A.stats(k);drawer.appendChild(el('p',s.hp+' KP · '+s.ang+' Angriff · '+s.tempo+' Tempo','gm-mon-stats'));
      ...
      else{drawer.appendChild(el('h3','Runenstufe '+k.upgrade+'/'+X.UPGRADE_LIMIT));drawer.appendChild(el('p','+'+Math.round(k.upgrade*A.UPGRADE_BONUS*100)+' % KP und Angriff · maximal +'+Math.round(X.UPGRADE_LIMIT*A.UPGRADE_BONUS*100)+' %. Ab Stufe '+A.SCHNELL_AB+' lädt der Kraftschlag eine Runde schneller, ab Stufe '+A.LADUNG_AB+' gibt es eine dritte Ladung der Fähigkeit. Tempo bleibt gleich.'));drawer.appendChild(el('p','Jetzt: '+A.ladungen(k)+' Ladungen · Kraftschlag alle '+A.powerPause(k)+' Runden.'));drawer.appendChild(el('p','Passende Runen: '+st.runes[k.seltenheit]+' × '+D.SELTENHEITEN[k.seltenheit].name));var rune=button(k.upgrade===5?'Maximale Runenstufe':'Verbessern · '+(k.upgrade+1)+' passende Rune(n)',function(){perform('mon_upgrade',{monId:id,level:k.upgrade},'mons');},'gm-button gm-primary');rune.disabled=k.upgrade>=5||st.runes[k.seltenheit]<k.upgrade+1;drawer.appendChild(rune);planTeil(id,k);drawer.appendChild(el('h3','Aufstellung ändern'));drawer.appendChild(el('p','Deine Aufstellung wird für Angriffe und die Verteidigung deiner Außenposten gespeichert.'));st.truppe.forEach(function(old,i){var b=button((i+1)+'. Platz: '+D.mon(old).name,function(){var squad=st.truppe.slice(),at=squad.indexOf(id);if(at>=0)squad[at]=old;squad[i]=id;perform('defend',{squad:squad},'mons');});b.disabled=old===id;drawer.appendChild(b);});}
    }
