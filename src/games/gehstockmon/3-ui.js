...
(function (SG) {
  ...
  function mount(host) {
    ...
    function syncPresence(){if(dead||!connected||presenceBusy||busy||document.hidden||!world||!world.position)return;presenceBusy=true;lastPresence=Date.now();var position=world.position();R.online.request('presence',{position:position}).then(function(res){if(dead||!connected)return;presenceFailed=false;lastPresencePos=res.positionCorrected&&res.position?res.position:position;setAccess(res.access);if(res.positionCorrected&&world.setPosition)world.setPosition(res.position);lastPresenceReply=Date.now();applyPeers(res.peers||[],res.serverTime);if(res.duellEinladung)duellUi.einladung(res.duellEinladung);}).catch(function(err){presenceFailed=true;if(err.status===423){onlineError(err);return;}if(!dead&&Date.now()-lastPresenceReply>15000){applyPeers([],now());worldButton.textContent='Spielerwelt · Verbindung prüfen';}}).finally(function(){presenceBusy=false;});}
