...
await test('Sandbox purchases, fights, presence, retries and reload never access live stores',async()=>{
  let time=mon;const forbidden={getWithMetadata(){throw Error('live read');},setJSON(){throw Error('live write');}},handler=createHandler({store:forbidden,presenceStore:forbidden,now:()=>time});let serial=0;
  async function call(op,extra={}){const response=await handler(new Request('http://localhost/api/gehstockmon',{method:'POST',body:JSON.stringify({code:'0141',op,requestId:'sandbox-'+(++serial),adminOverride:true,adminCode:'3141',...extra})}));assert.equal(response.status,200,await response.clone().text());return response.json();}
  let r=await call('join');assert.equal(r.profile.gold,50000);assert.equal('testState' in r,false);r=await call('shop_buy',{kind:'skin',itemId:'leerenreaper',requestId:'sandbox-purchase'});assert.equal(r.profile.gold,45000);assert.equal((await call('shop_buy',{kind:'skin',itemId:'leerenreaper',requestId:'sandbox-purchase'})).profile.gold,45000);
  await call('presence',{position:{...X.SPAWN,heading:0}});r=await call('arena_start',{territoryId:9,version:1,squad:['endrichter','aetherdrache','titanenkrone','sonnenkoenig']});assert.equal(r.arena.teams[1][0].monId,'endrichter');
  r=await call('join');assert.equal(r.profile.gold,45000);assert.ok(r.arena,'the shared test session survives reloads');time+=300001;r=await call('join');assert.equal(r.profile.gold,50000);assert.equal(r.profile.skin,'wanderer');assert.equal(r.arena,null);
});
