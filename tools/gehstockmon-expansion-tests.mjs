...
await test('Sandbox purchases, fights, presence, retries and reload never access live stores',async()=>{
  ...
  await call('presence',{position:{...X.SPAWN,heading:0}});r=await call('arena_start',{territoryId:9,version:1,squad:['endrichter','nullwyrm','aetherdrache','chronoschreiter']});assert.equal(r.arena.teams[1][0].monId,'endrichter');
