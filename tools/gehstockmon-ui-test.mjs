...
export async function checkUi(D,E,A,handler,code,clock){
  ...
  let definition,timerId=0,blocked=false,pauses=[],latest,unreachable=true,loseResponse=false,requests=[];
  const stage=new Element('div'),root=new Element('div');root.appendChild(stage);
  const world={setHeld(){},setSquad(){},setTerritories(){},select(){},follow(){},distanceTo:()=>0,blockInput:yes=>{blocked=yes;},move(){},pause:yes=>pauses.push(yes),destroy(){}};
  const SG={gehstockmon:{daten:D,wirtschaft:E,arena:A,orte:D.FELDER.map(()=>({terrain:'Wiese'})),createWorld:()=>world},util:{},storage,auth:{aktuell:{code,name:'UI Test'}},offline:false,env:{},assets:{},register:def=>{definition=def;}};
  ...
  const context=vm.createContext({SG,document,window:{addEventListener:(k,fn)=>listeners[k]=fn,removeEventListener:k=>delete listeners[k]},Date:class extends Date{static now(){return clock.value;}},AbortController,setTimeout,clearTimeout,fetch:async(url,opts)=>{
    ...
    const response=await handler(new Request('http://localhost'+url,opts));if(response.ok)latest=await response.clone().json();
