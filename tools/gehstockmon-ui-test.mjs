...
export async function checkUi(D,E,A,handler,code,clock,otherCode){
  ...
  let position={x:0,z:125},definition,timerId=0,blocked=false,pauses=[],latest,unreachable=true,loseResponse=false,requests=[],visiblePeers=[],worldFrame,delayReply=false,releaseReply;
  ...
  for(const file of['src/core/ui.js','src/games/gehstockmon/1-zeiten.js','src/games/gehstockmon/1-zusatz.js','src/games/gehstockmon/2-figuren.js','src/games/gehstockmon/2-online.js','src/games/gehstockmon/2-abenteuer-ui.js','src/games/gehstockmon/3-ui.js'])vm.runInContext(fs.readFileSync(file,'utf8'),context);
  ...
  await handler(new Request('http://localhost/api/gehstockmon',{method:'POST',body:JSON.stringify({code:otherCode,op:'presence',position:{x:0,z:125,heading:0}})}));
