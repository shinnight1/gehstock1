...
world.setPeers([{id:'friend',name:'Friend',x:20,z:10,heading:-1,updatedAt:12000,activity:'map'}],12000);
for(let i=0;i<60;i++)loop.update(1/60);assert.ok(peer.position.x>10&&peer.position.x<20,'remote movement is interpolated');
let releasedPeer=false;peerTexture.addEventListener('dispose',()=>releasedPeer=true);world.setPeers([],13000);assert.ok(releasedPeer);assert.equal(scene.getObjectByName('peer-friend'),undefined);
