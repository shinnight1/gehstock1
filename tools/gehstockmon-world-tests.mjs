...
// Remote players stay three-dimensional while their full skin models load.
...
const loadingPeer=scene.getObjectByName('peer-friend');assert.equal(loadingPeer.userData.portrait.visible,false,'remote pixel portrait stays hidden');assert.ok(loadingPeer.getObjectByName('peer-3d-placeholder'),'remote player has a 3D figure while its skin model loads');
