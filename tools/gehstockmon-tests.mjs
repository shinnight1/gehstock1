...
await test('A second copy raises the rune level and only becomes runes at the cap',()=>{
  const p=D.neuerStand(null,stamp),egg=(n)=>{p.eggs=[{id:'egg'+n,territoryId:1,startedAt:stamp,readyAt:stamp+E.HOUR,producedAt:stamp}];return E.hatch(p,'egg'+n,stamp+E.HOUR,0);};
  /* Ziehung 0 trifft immer dasselbe erste Mon des Vorrats: einmal neu, danach lauter Zwillinge. */
  const erst=egg('neu');assert.equal(erst.neu,true);const doppelt=erst.mon;
