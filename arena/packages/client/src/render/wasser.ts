...
const WELLEN: readonly Welle[] = [
  { lage: 0.18, dichte: 3.3, hub: 0.05, tempo: 0.5, breite: 0.03, farbe: FARBE.wasserGlanz },
  { lage: 0.36, dichte: 2.1, hub: 0.08, tempo: -0.34, breite: 0.042, farbe: FARBE.wasserHell },
  { lage: 0.58, dichte: 4.2, hub: 0.04, tempo: 0.66, breite: 0.026, farbe: FARBE.wasserGlanz },
  { lage: 0.76, dichte: 2.7, hub: 0.07, tempo: -0.22, breite: 0.038, farbe: FARBE.wasserHell },
  { lage: 0.88, dichte: 5.1, hub: 0.03, tempo: 0.88, breite: 0.022, farbe: FARBE.wasserGlanz },
];
...
export function wasserZeichnen(
  c: CanvasRenderingContext2D, k: Kamera, zeit: number,
): void {
  ...
  tiefeVerlauf.addColorStop(0, FARBE.wasser);
  tiefeVerlauf.addColorStop(0.5, FARBE.wasserTief);
  tiefeVerlauf.addColorStop(1, FARBE.wasser);
  ...
  schaumkante(c, k, FLUSS_OBEN, zeit, 1);
  schaumkante(c, k, FLUSS_UNTEN, zeit, -1);
  c.restore();
}
...
function schaumkante(
  ...
  zeit: number, richtung: number,
): void {
  const y = kante + tile(0.05) * richtung;
  c.strokeStyle = FARBE.wasserSchaum;
